...
type LikedPositionList struct {
	DB             *sql.DB
	Level          int
	Total          int             `json:"total"`
	LikedPositions []LikedPosition `json:"LikedPosition"`
}

type LikedPosition struct {
	Id         int64  `json:"id"`
	UserId     int64  `json:"user_id"`
	PublishNo  string `json:"publish_no"`
	InsertTime string `json:"insert_time"`
...
type Form struct {
	Form LikedPosition `json:"LikedPosition"`
}
...
func New(db *sql.DB, level int) *LikedPositionList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &LikedPositionList{DB: db, Total: 0, LikedPositions: make([]LikedPosition, 0), Level: level}
}
...
func NewUrl(url string, level int) *LikedPositionList {
	var err error
...
func (r *LikedPositionList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
	...
	qrySql := fmt.Sprintf("Select count(1) as total from tba_account_liked_postions   where 1=1 %s", where)
	if r.Level == DEBUG {
...
func (r LikedPositionList) Get(s Search) (*LikedPosition, error) {
	var where string
	l := time.Now()
...
func (r *LikedPositionList) GetList(s Search) ([]LikedPosition, error) {
	var where string
	l := time.Now()
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		...
		qrySql = fmt.Sprintf("Select id,user_id,publish_no,insert_time,update_time,version from tba_account_liked_postions where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	for rows.Next() {
		...
		r.LikedPositions = append(r.LikedPositions, p)
	}
	log.Println(SQL_ELAPSED, r)
	if r.Level == DEBUG {
		log.Println(SQL_ELAPSED, time.Since(l))
	}
	return r.LikedPositions, nil
}
...
func (r *LikedPositionList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r LikedPositionList) Insert(p LikedPosition) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  tba_account_liked_postions(user_id,publish_no,version)  values(?,?,?,?,?,?)")
	if r.Level == DEBUG {
...
func (r LikedPositionList) InsertEntity(p LikedPosition, tr *sql.Tx) error {
	l := time.Now()
	...
	exeSql := fmt.Sprintf("Insert into  tba_account_liked_postions(%s)  values(%s)", colNames, colTags)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r LikedPositionList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r LikedPositionList) UpdataEntity(keyNo string, p LikedPosition, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
	...
	exeSql := fmt.Sprintf("update  tba_account_liked_postions  set %s  where id=? ", colNames)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r LikedPositionList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	...
	valSlice = append(valSlice, keyNo)
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update tba_account_liked_postions set %s where id=?", colNames)
	if r.Level == DEBUG {
...
func (r LikedPositionList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  tba_account_liked_postions  where id=?")
	if r.Level == DEBUG {
