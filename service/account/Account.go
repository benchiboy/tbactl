...
type Search struct {
	...
	PartnerOpenId string  `json:"partner_open_id"`
...
func (r *AccountList) GetTotal(s Search) (int, error) {
	...
	if s.PartnerOpenId != "" {
		where += " and partner_open_id='" + s.PartnerOpenId + "'"
	}
	...
	if s.SessionKey != "" {
		where += " and session_key='" + s.SessionKey + "'"
	}
...
func (r AccountList) Get(s Search) (*Account, error) {
	...
	qrySql := fmt.Sprintf("Select id,user_id,partner_open_id,partner_user_id,parent_user_id,session_key,user_role,user_status,avatar_url,login_mode,login_name,login_password,nick_name,gender,city,province,country,language,error_count,account_bal,goods_count,market,user_channel,random_no,region_no,customer_id,memo,insert_user,update_user,version from tba_accounts where 1=1 %s ", where)
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	rows, err := r.DB.Query(qrySql)
	...
	if !rows.Next() {
		...
		err := rows.Scan(&p.Id, &p.UserId, &p.PartnerOpenId, &p.PartnerUserId, &p.ParentUserId, &p.SessionKey, &p.UserRole, &p.UserStatus, &p.AvatarUrl, &p.LoginMode, &p.LoginName, &p.LoginPassword, &p.NickName, &p.Gender, &p.City, &p.Province, &p.Country, &p.Language, &p.ErrorCount, &p.AccountBal, &p.GoodsCount, &p.Market, &p.UserChannel, &p.RandomNo, &p.RegionNo, &p.CustomerId, &p.Memo, &p.InsertUser, &p.UpdateUser, &p.Version)
		if err != nil {
			log.Println(SQL_ERROR, err.Error())
			return nil, err
		}
...
func (r *AccountList) GetList(s Search) ([]Account, error) {
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		qrySql = fmt.Sprintf("Select id,user_id,partner_open_id,partner_user_id,parent_user_id,session_key,user_role,user_status,avatar_url,login_mode,login_name,login_password,nick_name,gender,city,province,country,language,error_count,account_bal,goods_count,market,user_channel,random_no,region_no,customer_id,created_time,updated_time,memo,insert_user,update_user,version from tba_accounts where 1=1 %s", where)
	} else {
		qrySql = fmt.Sprintf("Select id,user_id,partner_open_id,partner_user_id,parent_user_id,session_key,user_role,user_status,avatar_url,login_mode,login_name,login_password,nick_name,gender,city,province,country,language,error_count,account_bal,goods_count,market,user_channel,random_no,region_no,customer_id,created_time,updated_time,memo,insert_user,update_user,version from tba_accounts where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	var p Account
	for rows.Next() {
		rows.Scan(&p.Id, &p.UserId, &p.PartnerOpenId, &p.PartnerUserId, &p.ParentUserId, &p.SessionKey, &p.UserRole, &p.UserStatus, &p.AvatarUrl, &p.LoginMode, &p.LoginName, &p.LoginPassword, &p.NickName, &p.Gender, &p.City, &p.Province, &p.Country, &p.Language, &p.ErrorCount, &p.AccountBal, &p.GoodsCount, &p.Market, &p.UserChannel, &p.RandomNo, &p.RegionNo, &p.CustomerId, &p.CreatedTime, &p.UpdatedTime, &p.Memo, &p.InsertUser, &p.UpdateUser, &p.Version)
...
func (r AccountList) Insert(p Account) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  tba_accounts(user_id,partner_open_id,partner_user_id,parent_user_id,session_key,user_role,user_status,avatar_url,login_mode,login_name,login_password,nick_name,gender,city,province,country,language,error_count,account_bal,goods_count,market,user_channel,random_no,region_no,customer_id,created_time,updated_time,memo,insert_user,update_user,version)  values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.UserId, p.PartnerOpenId, p.PartnerUserId, p.ParentUserId, p.SessionKey, p.UserRole, p.UserStatus, p.AvatarUrl, p.LoginMode, p.LoginName, p.LoginPassword, p.NickName, p.Gender, p.City, p.Province, p.Country, p.Language, p.ErrorCount, p.AccountBal, p.GoodsCount, p.Market, p.UserChannel, p.RandomNo, p.RegionNo, p.CustomerId, p.CreatedTime, p.UpdatedTime, p.Memo, p.InsertUser, p.UpdateUser, p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r AccountList) InsertEntity(p Account, tr *sql.Tx) error {
	...
	if p.PartnerOpenId != "" {
		colNames += "partner_open_id,"
		colTags += "?,"
		valSlice = append(valSlice, p.PartnerOpenId)
	}
	...
	if p.SessionKey != "" {
		colNames += "session_key,"
		colTags += "?,"
		valSlice = append(valSlice, p.SessionKey)
	}
