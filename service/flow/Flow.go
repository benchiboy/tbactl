...
func (r AccountFlowsList) Get(s Search) (*AccountFlows, error) {
	...
	qrySql := fmt.Sprintf("Select user_id,trxn_no,trxn_date,trxn_amt,trxn_type,proc_status,proc_msg,goods_no,discount_rate,promotion_no,account_bal,trxn_memo,done_date,update_user,version from tba_account_flows where 1=1 %s ", where)
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	rows, err := r.DB.Query(qrySql)
	...
	var p  AccountFlows
	for rows.Next() {
		rows.Scan(&p.UserId,&p.TrxnNo,&p.TrxnDate,&p.TrxnAmt,&p.TrxnType,&p.ProcStatus,&p.ProcMsg,&p.GoodsNo,&p.DiscountRate,&p.PromotionNo,&p.AccountBal,&p.TrxnMemo,&p.DoneDate,&p.UpdateUser,&p.Version)
		break
	}
...
func (r AccountFlowsList) InsertEntity(p AccountFlows) error {
	l := time.Now()
...
func (r AccountFlowsList) InsertMap(m map[string]interface{}) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r AccountFlowsList) UpdateMap(keyNo string, m map[string]interface{}) error {
	l := time.Now()
	...
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update tba_account_flows set %s where trxn_no=?", colNames)
...
func (r AccountFlowsList) Delete(keyNo string) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  tba_account_flows  where trxn_no=?")
	if r.Level == DEBUG {
